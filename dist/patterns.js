 ...
 */
export const PATTERN_DESCRIPTORS = [
    {
        title: "Structured Error Handling",
        description: "Wrap risky operations with explicit try/catch (or equivalent) and return actionable errors.",
        regexes: [/\btry\s*\{/, /\bcatch\s*\(/, /\bexcept\s+/, /\bif\s+err\s*!=\s*nil\b/],
    },
    {
        title: "Input Validation at Boundaries",
        description: "Validate external inputs early using schemas/assertions before executing business logic.",
        regexes: [
            /\bvalidate\w*\s*\(/i,
    ...
    },
    {
        ...
        description: "Use async/await or concurrency primitives to handle network and IO without blocking.",
        regexes: [
            /\basync\b/,
            /\bawait\b/,
            /\bPromise\.all\b/,
    ...
    },
    {
        ...
        description: "Add retries with delay/backoff when calling external services to improve resilience.",
        regexes: [/\bretry\b/i, /\bbackoff\b/i, /\bexponential\b/i, /\bsleep\s*\(/i],
    },
    {
        title: "Clear Client Abstraction",
        description: "Keep transport/API concerns in dedicated client helpers instead of spreading calls everywhere.",
        regexes: [
    ...
    },
    {
        ...
        description: "Use in-memory or shared caches to avoid repeated expensive requests/computations.",
 ...
 */
export function sanitizeSnippet(snippet, maxLength = 180) {
 ...
 */
export function firstMatchingLine(content, regexes) {
 ...
 */
export function derivePatternSuggestions(files) {
 ...
 */
export function deriveFallbackSuggestions(files) {
