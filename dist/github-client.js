 ...
 */
function buildSearchQuery(problem, language) {
...
async function githubFetchJson(url) {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401) {
            throw new Error("GitHub API 401: authentication failed. Ensure a valid GITHUB_TOKEN (or GITHUB_PAT) is set for the MCP server process.");
        }
 ...
 */
export async function searchGithubCode(problem, language, perPage) {
    ...
    const { data, headers } = await githubFetchJson(url);
    return {
        query,
...
async function fetchFileContent(item) {
    try {
        const { data } = await githubFetchJson(item.url);
