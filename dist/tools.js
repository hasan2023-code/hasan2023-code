import { searchGithubCode, fetchTopFiles, uniqueCount, hasGitHubToken, } from "./github-client.js";
import { derivePatternSuggestions, deriveFallbackSuggestions, sanitizeSnippet, } from "./patterns.js";
// ============================================================================
// MCP TOOL HANDLERS
// ============================================================================
/**
...
export async function runSearchTool(args) {
    ...
    if (!hasGitHubToken()) {
        lines.push("");
        lines.push("Tip: set GITHUB_TOKEN (or GITHUB_PAT) to increase search rate limits and reliability.");
    }
    return {
        text: lines.join("\n"),
        structured: {
 ...
 */
export async function runSuggestTool(args) {
    const search = await searchGithubCode(args.problem, args.language, args.perPage);
    if (search.items.length === 0) {
        return {
    ...
    if (!hasGitHubToken()) {
        lines.push("");
        lines.push("Tip: set GITHUB_TOKEN (or GITHUB_PAT) to reduce rate-limit errors during pattern extraction.");
    }
    return {
        text: lines.join("\n"),
        structured: {
